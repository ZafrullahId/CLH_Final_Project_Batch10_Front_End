...
function disabledbutton() {
    ...
    // fetch(`https://localhost:5001/api/RawMaterial/GetRawAsync/${but[i].id[2]}`)
    //     .then(res => res.json())
    //     .then(data => btn.push(data.data));

    // for (let i = 0; i < btn.length; i++) {
    //     console.log(btn.length)
    //     console.log(btn)
    //     if (btn[i].enumApprovalStatus == 1 || btn[i].enumApprovalStatus == 3) {
    //         let dis = document.querySelectorAll("#id" + btn[i].id)
    //         console.log(dis)
    //         console.log("break")
    //         dis.forEach(x => {
    //             console.log(i)
